...
/** Resolve a filename to an absolute path INSIDE the vault, rejecting
 *  any attempt to escape it via '..' or absolute paths. */
function resolveSafe(filename) {
  const target = path.resolve(VAULT_DIR, filename);
  if (!target.startsWith(VAULT_DIR + path.sep) && target !== VAULT_DIR) {
    throw new Error(`Blocked: "${filename}" resolves outside the vault sandbox.`);
  }
  return target;
}
